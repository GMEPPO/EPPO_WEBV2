...
class ProposalsManager {
    ...
    _buildPedidoEncomendaFormStep(proposal, selectedArticuloIds) {
        ...
        function buildPropostaDocPreview(url) {
            ...
            if (isImg) return '<div style="width:56px;height:56px;border-radius:6px;overflow:hidden;background:#334155;flex-shrink:0;display:flex;align-items:center;justify-content:center;position:relative;"><img src="' + safe + '" alt="" style="width:100%;height:100%;object-fit:cover;position:absolute;inset:0;" onerror="this.style.display=\'none\';var s=this.nextElementSibling;if(s)s.style.display=\'flex\';"><span style="display:none;position:absolute;inset:0;align-items:center;justify-content:center;color:#94a3b8;"><i class="fas fa-image"></i></span></div>';
            const isPdf = /\.pdf(\?|$)/.test(path);
            const icon = isPdf ? 'fa-file-pdf' : 'fa-file';
            const color = isPdf ? '#ef4444' : '#94a3b8';
            return '<div style="width:56px;height:56px;border-radius:6px;background:#334155;display:flex;align-items:center;justify-content:center;flex-shrink:0;"><i class="fas ' + icon + '" style="font-size:1.5rem;color:' + color + ';"></i></div>';
        }
        if (this.allProducts && this.allProducts.length > 0) {
            this.allProducts.forEach(p => {
                if (p.id) productMap[p.id] = p;
        ...
        articulos.forEach((articulo, index) => {
            ...
            row.innerHTML = `
                ...
                <div style="flex: 1; min-width: 0;">
                    ...
                    <div class="pedido-encomenda-main-fields" style="margin-bottom: 12px; padding: 14px; background: rgba(59, 130, 246, 0.1); border: 2px solid rgba(59, 130, 246, 0.4); border-radius: 10px;">
                        <div style="font-size: 0.75rem; font-weight: 600; color: var(--primary, #3b82f6); margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.02em;">${lang === 'es' ? 'Cantidad, precio y descuento' : lang === 'en' ? 'Quantity, price & discount' : 'Quantidade, preço e desconto'}</div>
    ...
    _addPedidoEncomendaExternalRow() {
        ...
        row.innerHTML = `
            ...
            <div style="flex: 1; min-width: 0;">
                ...
                <div class="pedido-encomenda-main-fields" style="margin-bottom: 12px; padding: 14px; background: rgba(59, 130, 246, 0.1); border: 2px solid rgba(59, 130, 246, 0.4); border-radius: 10px;">
                    <div style="font-size: 0.75rem; font-weight: 600; color: var(--primary, #3b82f6); margin-bottom: 10px; text-transform: uppercase;">${L.mainFieldsLabel}</div>
                    <div style="display: flex; align-items: flex-end; gap: 20px; flex-wrap: wrap;">
        ...
        for (let i = 0; i < articulos.length; i++) {
            ...
            if (!hasPhc) {
                ...
                if (row.personalizado) {
                    ...
                    document.querySelectorAll('.ge-proposta-doc-cb[data-doc-url]').forEach(cb => {
                        if (cb.id && cb.id.startsWith('ge-proposta-doc-' + articuloId + '-') && cb.checked) {
                            const u = cb.getAttribute('data-doc-url');
                            if (u) selectedPropostaDocs.push(u);
                        }
                    });
                    row.personalizado_anexos_urls = selectedPropostaDocs.slice(0, 4);
