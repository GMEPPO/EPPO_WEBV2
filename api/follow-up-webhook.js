/**
 * Proxy para el webhook n8n. Solo reenvía si tipo_alerta es permitido.
 * Ruta: POST /api/follow-up-webhook (rewrite puede enviar aquí).
 * Alertas follow-up / aguarda_* no se reenvían a n8n.
 */
...
module.exports = async function handler(req, res) {
    ...
    if (!TIPOS_ALERTA_PERMITIDOS.has(tipoAlerta)) {
        return res.status(200).json({ ok: true, skipped: true, reason: 'tipo_alerta no permitido' });
    }
